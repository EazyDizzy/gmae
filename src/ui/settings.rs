...
pub fn update_background_music<'settings>(
    game_settings: &'settings mut ResMut<GameSettings>,
) -> Box<dyn FnMut(Option<f64>) -> f64 + 'settings> {
...
pub fn update_mouse_sensitivity<'settings>(
    game_settings: &'settings mut ResMut<GameSettings>,
) -> Box<dyn FnMut(Option<f64>) -> f64 + 'settings> {
    Box::new(move |new_value: Option<f64>| -> f64 {
        match new_value {
